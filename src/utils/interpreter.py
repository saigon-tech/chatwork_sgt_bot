...
def interpret_message(message):
    try:
        valid_intents = ActionRegistry.get_all_intents() + [
            "greeting",
            "unknown",
        ]
        intent_descriptions = {
            intent: ActionRegistry._actions.get(intent, {}).get("description", "")
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Extract the intent from this message: '{message}'",
                },
            ],
            max_tokens=50,
        )
