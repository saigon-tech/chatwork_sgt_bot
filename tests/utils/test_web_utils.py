...
def test_fetch_url_text(web_helper):
    ...
    with patch.object(web_helper, "fetch_url_html") as mock_fetch_html:
        mock_fetch_html.return_value = (
            "<html><body><p>Test paragraph 1</p><p>Test paragraph 2</p></body></html>"
        )
...
def test_fetch_url_text_max_chars(web_helper):
    ...
    with patch.object(web_helper, "fetch_url_html") as mock_fetch_html:
        mock_fetch_html.return_value = (
            "<html><body><p>Test content</p><p>More content</p></body></html>"
        )
...
def test_fetch_url_text_error(web_helper):
    with patch.object(
        web_helper, "fetch_url_html", side_effect=Exception("Processing error")
    ):
        with pytest.raises(Exception) as exc_info:
